...
@st.cache_data(show_spinner=False)
def compute_q1_q2_q3(dfs: dict, order_merge: pd.DataFrame) -> dict:
...
@st.cache_data(show_spinner=False)
def compute_q4_q5(dfs: dict, order_merge: pd.DataFrame) -> dict:
    ...
    # Visualisasi segmentasi RFM (High= Sultan)
...
def main():
    ...
    with st.spinner("Computing metrics..."):
        ...
        q1_q2_q3 = compute_q1_q2_q3(dfs, order_merge)
        q4_q5 = compute_q4_q5(dfs, order_merge)
    ...
    ax.set_xticks(list(yearly_revenue["year"].unique()))
    fig.tight_layout()
    st.pyplot(fig, use_container_width=True)

    # Q2
    st.subheader("Q2. Dibulan apa penjualan cenderung meningkat")
    monthly_revenue = q1_q2_q3["monthly_revenue"].copy()
    fig, ax = plt.subplots(figsize=(10, 4))
    sns.lineplot(data=monthly_revenue, x="year_month", y="revenue", marker="o", ax=ax)
    ax.set_title("Revenue per Bulan (sum(price))")
    ax.tick_params(axis="x", rotation=45)
    fig.tight_layout()
    st.pyplot(fig, use_container_width=True)

    month_trend = q1_q2_q3["month_trend"].copy()
    month_name_id = q1_q2_q3["month_name_id"]
    fig, ax = plt.subplots(figsize=(10, 4))
    sns.barplot(
    ...
    # Q3
    st.subheader("Q3. Apa 3 product terlaris setiap bulan-nya")
    top3_plot = q1_q2_q3["top3_plot"].copy()
    ...
    ax.tick_params(axis="x", rotation=45)
    ax.set_title("Pertanyaan 3: Top 3 Kategori Produk Terlaris per Bulan Kalender (Tanpa Tahun)")
    ...
    winner_counts = q1_q2_q3["winner_counts"]
    fig, ax = plt.subplots(figsize=(8, 4))
    sns.barplot(x=winner_counts.values, y=winner_counts.index, orient="h", ax=ax)
    ax.set_title("Kategori #1 yang Paling Sering Menjadi Terlaris (Top-1 per Bulan)")
    ax.set_xlabel("Jumlah Bulan Menjadi #1")
    ...
    st.caption(
        ...
        f"(weighted = 5*Monetary + 5*Frequency + 2*Recency)."
    )
    ...
    st.caption(
        f"State terbaik (avg RFM tertinggi): **{q4_q5['best_state']}** "
        f"(avg_rfm={q4_q5['best_rfm']:.4f}, total_customers={q4_q5['best_count']})"
    )
    st.plotly_chart(q4_q5["q5_fig"], use_container_width=True)
